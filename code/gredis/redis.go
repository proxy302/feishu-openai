...
func Setup() error {
	RedisConn = &redis.Pool{
		MaxIdle:     initialization.GetConfig().RedisMaxIdle,
		MaxActive:   initialization.GetConfig().RedisMaxActive,
		IdleTimeout: time.Duration(initialization.GetConfig().RedisIdleTime) * time.Second,
		Wait:        true,
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", fmt.Sprintf("%s:%d", initialization.GetConfig().RedisHost, initialization.GetConfig().RedisPort), redis.DialPassword(initialization.GetConfig().RedisPassword))
			if err != nil {
				return nil, err
			}
			return c, err
...
func Rpush(key string, value interface{}) (int, error) {
	conn := RedisConn.Get()
	defer conn.Close()
	reply, err := redis.Int(conn.Do("RPUSH", redis.Args{}.Add(key).AddFlat(value)...))
	if err != nil {
		return reply, err
	}
	return reply, nil
